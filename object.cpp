...
GameObject::GameObject(std::string textureFile) {
    ...
    light = Shader("shaders/lighting/cube.vs", "shaders/lighting/cube.fs");
    shader = Shader("shaders/lighting/light.vs", "shaders/lighting/light.fs");
    shader.setInt("texture1", 0);
    LoadTexture(textureFile);
}

GameObject::GameObject(std::vector<std::string> faces) {
    SetType("skybox");
    shader = Shader("shaders/skybox/vertex.vs", "shaders/skybox/fragment.fs");
...
void GameObject::LoadCubemap(std::vector<std::string> faces) {
    float vertices[] = {
...
void GameObject::Render() {   
    if (GetType() == "skybox") {
        ...
        glm::mat4 projection = glm::perspective(glm::radians(80.0f), (float)1920 / (float)1080, 0.1f, 100.0f);
    ...
    shaderModel = glm::scale(shaderModel, scale);
    shaderModel = glm::translate(shaderModel, position);
    shaderModel = glm::rotate(shaderModel, rotation.x, glm::vec3(-1.0f, 0.0f, 0.0f));
